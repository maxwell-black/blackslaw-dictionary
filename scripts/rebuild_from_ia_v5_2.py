...
def build_rebuilt_entries(
    ...
    for idx, entry in enumerate(current_entries):
        if idx in mapping:
            ...
            # Fallback: if source body is empty but original has content, use cleaned original
            if not body.strip() and entry.get("body", "").strip():
                body = clean_orphan_numerics(entry["body"])
        else:
            match_score = 0.0
